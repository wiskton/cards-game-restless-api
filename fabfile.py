...
repositorio = 'git@bitbucket.org:willemarf/cards.git'
...
def celery():
    """celery local"""
    local('python manage.py celeryd -l info')
...
def cw():
    ...
    local('compass watch cards/static')

def compass_compile():
    """compila o compassa remoto"""
    log('Compilando arquivos SASS')
    with cd(env.project_path):
        run('compass compile -e production cards/static')
...
def translate():
    ...
    local('django-admin.py makemessages --locale=en --ignore=templates/admin --ignore=cards/settings.py')
